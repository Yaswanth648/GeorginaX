...
public class covidData extends Bot implements Master {
    ...
    public void handleRequests(Update update, String cmd) {
        if (cmd.equalsIgnoreCase(getHandler() + "covid")) {
            try {
                HttpResponse<String> response = Unirest.get("https://covid-19-data.p.rapidapi.com/country?name=india")
                        .header("x-rapidapi-key", configutation.rapidXapi)
                        .header("x-rapidapi-host", "covid-19-data.p.rapidapi.com").asString();
                // response.getBody();
                response.getBody();
